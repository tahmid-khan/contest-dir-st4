...
struct Hash {
	uint64_t operator()(int64_t x) const { return __builtin_bswap64((x ^ ticks_since_epoch) * large_odd); }
};

#ifdef __GLIBCXX__  // using libstdc++
	template<typename T> using Hash_map = __gnu_pbds::gp_hash_table<int64_t, T, Hash>;
	// Ctor signature: gp_hash_table(const hash_fn& h, const eq_fn& e, const comb_probe_fn& cp, const probe_fn& p, const resize_policy& rp)
